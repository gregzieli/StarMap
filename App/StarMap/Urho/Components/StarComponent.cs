...
namespace StarMap.Urho.Components
{
  public class StarComponent : StaticSprite2D
  {
    ...
    public async void Highlight()
    {
      ...
      _scale = Node.Scale;
      // "blinking" animation
      await Node.RunActionsAsync(new RepeatForever(new ScaleTo(1, 0.9f), new ScaleTo(1, 2)));
    }
    ...
    public async void Dim()
    {
      ...
      await Node.RunActionsAsync(new EaseBackOut(new ScaleTo(1f, _scale.X, _scale.Y, _scale.Z)));
    }
    ...
    public async void Select()
    {
      ...
      await selectionNode.RunActionsAsync(new RepeatForever(new RotateBy(1, 0, 0, 35)));
    }
